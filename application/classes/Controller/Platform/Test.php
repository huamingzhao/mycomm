...
class Controller_Platform_Test extends Controller_Platform_Template{
    ...
    public function action_index(){
    	...
    	$invest = new Service_Platform_Invest();
    	if (empty($searchrecomandInvest)&&arr::get($search, 'from')==""&&Arr::get($search, 'page')==""){
	    	$memcache = Cache::instance ( 'memcache' );
	        $investment_show_date = $memcache->get($this->_investment_show_date);
	        if (empty($investment_show_date)){
	        	$memcache->set($this->_investment_show_date, date('Y-m-d'), $this->_cache_get_hour_time);
	        	$memcache->set($this->_investment_show,$invest->searchPlatformInvestment($searchrecomandInvest,arr::get($search, 'from')),$this->_cache_get_hour_time);
	        }
	        else{
	        	$investment_show = $memcache->get($this->_investment_show);
	        	if ($investment_show_date==date('Y-m-d')){
	        		if (empty($investment_show)){
	        			$memcache->set($this->_investment_show_date, date('Y-m-d'), $this->_cache_get_hour_time);
	        			$memcache->set($this->_investment_show,$invest->searchPlatformInvestment($searchrecomandInvest,arr::get($search, 'from')),$this->_cache_get_hour_time);
	        		}
	        		else{
	        			$cache_result = $investment_show;
	        		}
	        	}
	        	else{
	        		$memcache->set($this->_investment_show_date, date('Y-m-d'), $this->_cache_get_hour_time);
...
//    	$result = $invest->searchPlatformInvestment($searchrecomandInvest,arr::get($search, 'from'));
    	...
    	if($result['count']==0){
    		...
    		$content->list = $invest->getResaultList($recomand[0]['list']);
    		$content->page = $recomand[0]['page'];
    		$count = $recomand[0]['count'];
    		$content->new = $recomand[1];
    	}
    	else{
    		$content->list = $invest->getResaultList($result['list']);
    	...
    	$this->template->title = "投资考察_一句话投资考察项目";
    	$this->template->keywords = "投资考察，考察项目，投资考察，一句话";
    	$this->template->description = "投资考察发布各种需要投资赚钱的好项目。火爆的考察项目一句话就能找到，投资项目赚钱考察上投资考察就可以了。";
    	
    	$total = 0;
    	if(Kohana::$performance){
    		foreach (Kohana::$performance as $p){
    			echo "消耗时间（单位微秒）：<span style='color:red;'>{$p[0]}</span> SQL:  {$p[1]}<br>";
    			if(strpos($p[2][0]['file'], "ORM.php")){
    			echo "ORM查询，暂无定位<br><br>";
    		}
    		else{
    			echo "<span style='color:green;'>DB查询,行数：{$p[2][0]['line']} ;文件名：".$p[2][0]['file']."</span><br><br>";
    			}
    				$total +=$p[0];
    			}
    			echo "SQL 总计消耗时间为 微秒<span style='color:red;'>{$total}</span><br><br>";
    			}
    					else{
    				echo "没有SQL执行";
    		}
...
// 	    	$content->industrynum = $IndustryNumBydata;
        ...
        if (empty($searchrecomandInvest)&&arr::get($search, 'from')==""&&Arr::get($search, 'page')==""){
        	$investment_show_date = $memcache->get($this->_investment_history_date);
        	if (empty($investment_show_date)){
        		$memcache->set($this->_investment_history_date, date('Y-m-d'), $this->_cache_get_hour_time);
        		$memcache->set($this->_investment_history,$service->searchHistoryInvestment($searchrecomandInvest,arr::get($search, 'from')),$this->_cache_get_hour_time);
        	}
        	else{
        		$investment_show = $memcache->get($this->_investment_history);
        		if ($investment_show_date==date('Y-m-d')){
        			if (empty($investment_show)){
        				$memcache->set($this->_investment_history_date, date('Y-m-d'), $this->_cache_get_hour_time);
        				$memcache->set($this->_investment_history,$service->searchHistoryInvestment($searchrecomandInvest,arr::get($search, 'from')),$this->_cache_get_hour_time);
        ...
        if ($cache_result!=""){
        	...
        	$result = $service->searchHistoryInvestment($searchrecomandInvest,arr::get($search, 'from'));
        }
//        $result = $service->searchHistoryInvestment($searchrecomandInvest,arr::get($search, 'from'));
        if($result['count']==0){
            $content->recomand = true;
            $recomand = $service->recomandHistoryInvest($searchrecomandInvest, $where,arr::get($search, 'from'));
