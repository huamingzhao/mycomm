 ...
 *
 */
class Controller_User_Person_Msg extends Controller_User_Person_Template{
/**
    * 查看名片相关用户消息
    * @author 龚湧
    */
    public function action_index(){
        ...
        //标记所有未读消息已读
        $service->viewMsg($this->userId(),$msg_group['person']['cards']);
        //生成或更新消息
        $service->generateMsg($user->user_id,$last_logintime,$user->user_type);

        $msg_list = $service->getMsgList($user->user_id,$msg_group['person']['cards']);

        //echo $service->getMsgCountByType($this->userId(),array(12));
        $content = View::factory("user/person/msg");
    ...
    public function action_auditStatus(){
        ...
        //标记所有未读消息已读
        $service->viewMsg($this->userId(),$msg_group['person']['audit']);

        $msg_list = $service->getMsgList($user->user_id,$msg_group['person']['audit']);

        $content = View::factory("user/person/msgauditstatus");
        $this->content->rightcontent = $content;
        //消息类表赋值
        $content->msg_list = $msg_list;
    }
    ...
    public function action_Project(){
        ...
        //标记所有未读消息已读
        $service->viewMsg($this->userId(),$msg_group['person']['project']);

        $msg_list = $service->getMsgList($user->user_id,$msg_group['person']['project']);


        $content = View::factory("user/person/msgproject");
        $this->content->rightcontent = $content;
        //消息类表赋值
        $content->msg_list = $msg_list;
    }
    ...
    public function action_read(){
        ...
        self::redirect("person/member/msg");
    }
    ...
    public function action_msglist(){
        ...
        $cache_name= "person_msg_group_type_array";
        $content = View::factory("user/person/newmsg");
        $this->content->rightcontent = $content;
        ...
        if( $stype=='all' || $stype=='' ){
            ...
            if( empty( $all_key ) ){
                $msg_group= message_type::msgGroup();
                $msg_group_person= $msg_group['person'];
                //生成全部的key
                $all_key= array();
        ...
        if( Arr::get($getarr, 'action')=='del' ){
            ...
            if( ceil($del_id)==0 ){
                self::redirect( '/person/member/msg/msglist' );
            }else{
            }
            //删除
            ...
            self::redirect( '/person/member/msg/msglist?stype='.$stype );
        }else{
        }
