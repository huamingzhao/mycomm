...
class Kohana_Request implements HTTP_Request {
    ...
    * 开启cookie 传送校验,默认为开启
    */
    public static $cookie_check = true;
    ...
    */
    public static $trusted_proxies = array('127.0.0.1', 'localhost', 'localhost.localdomain',
            '172.21.1.41','172.21.2.41','172.31.1.3');
    ...
    */
    public static function factory($uri = TRUE, $client_params = array(), $allow_external = TRUE, $injected_routes = array())
    {
        ...
        if ( ! Request::$initial)
        {
            ...
            if(isset($_SERVER["HTTP_CDN_SRC_IP"]) and $_SERVER["HTTP_CDN_SRC_IP"]){
                Request::$client_ip = $_SERVER["HTTP_CDN_SRC_IP"];
                if(isset($_GET['debug']) and $_GET['debug'] === 'ip'){
                    echo "ip_debug<br>";
            ...
            /*
             *增加了cookie校验，解决flash删除丢失cookie问题
             */
            if(strpos($uri, "/upload") === 0){
                Request::$cookie_check = false;//关闭cookie校验
            }
            if (self::$cookie_check and ($cookie_keys = array_keys($_COOKIE)))
            {
