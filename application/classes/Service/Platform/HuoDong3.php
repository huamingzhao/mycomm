 ...
 * @author 郁政
 */
class Service_Platform_HuoDong3{
...
//		$card_count = $card > 0 ? 1 : 0;
	...
	public function addDrawRecord($user_id,$prize_type,$mobile){
		...
		try {
			...
			$roulette->create();					
			if($prize_type == 1){				
				$msg = "恭喜您抽中iPod一个，我们将尽快为您寄送奖品。请将此消息分享给您的好友，一起来一句话商机网参与抽奖活动。如有疑问，敬请来电咨询400 1015 908";
				...
				$msg = "恭喜您抽中瑞士电脑双肩包一个，我们将尽快为您寄送奖品。请将此消息分享给您的好友，一起来一句话商机网参与抽奖活动。如有疑问，敬请来电咨询400 1015 908";
				$msg2 = common::send_message($mobile, $msg, 'online');
				//消息发送成功
	            if($msg2->retCode === 0){
	                $user_service->messageLog($mobile,$user_id,1,$msg,1);	                
				...
				$msg = "恭喜您抽中8g U盘一个，我们将尽快为您寄送奖品。请将此消息分享给您的好友，一起来一句话商机网参与抽奖活动。如有疑问，敬请来电咨询400 1015 908";
				$msg2 = common::send_message($mobile, $msg, 'online');
				//消息发送成功
	            if($msg2->retCode === 0){
	                $user_service->messageLog($mobile,$user_id,1,$msg,1);	                
				...
				$msg = "恭喜您抽中20元话费，活动结束时我们将尽快为您手机充值。请将此消息分享给您的好友，一起来一句话商机网参与抽奖。如有疑问，敬请来电咨询400 1015 908";
				$msg2 = common::send_message($mobile, $msg, 'online');
				//消息发送成功
	            if($msg2->retCode === 0){
	                $user_service->messageLog($mobile,$user_id,1,$msg,1);	                
				...
				$msg = "恭喜您抽中10元话费，活动结束时我们将尽快为您手机充值。请将此消息分享给您的好友，一起来一句话商机网参与抽奖。如有疑问，敬请来电咨询400 1015 908";
				$msg2 = common::send_message($mobile, $msg, 'online');
				//消息发送成功
	            if($msg2->retCode === 0){
	                $user_service->messageLog($mobile,$user_id,1,$msg,1);	                
	...
	public function getWinners(){
		...
		$om1 = DB::select('user_id','prize_type')->from('roulette')->where('prize_type','in',array(1,2,3,4,5))->where('game_id','=',$this->game_id)->order_by('lucky_time','desc')->limit(20)->execute()->as_array();
		$user_ids = array();
		$prize_types = array();
		$userInfo = array();
		if(!empty($om1)){
		...
		$prize_types = array();
		$userInfo = array();
		$om2 = DB::select('user_id','prize_type')->from('roulette')->where('prize_type','not in',array(1,2,3,4,5))->where('game_id','=',$this->game_id)->order_by('lucky_time','desc')->limit(80)->execute()->as_array();		
		if(!empty($om2)){
			foreach($om2 as $k => $v){
				$user_ids[] = $v['user_id'];
				$prize_types[] = $v['prize_type'];
	...
	public function getBigPrizeBefore($game_id){
		...
		$om = DB::select('user_id','prize_type')->from('roulette')->where('prize_type','in',array(1,2,3,4))->where('game_id','=',$game_id)->order_by('lucky_time','desc')->order_by('prize_type','asc')->limit(75)->execute()->as_array();
		$user_ids = array();
		$prize_types = array();
		$userInfo = array();
		if(!empty($om)){
	...
	public function getNumberByProbability($num,$game_id){
		...
		$amount_1 = isset($jiangpin_detail[1]['now_amount']) ? $jiangpin_detail[1]['now_amount'] : 3;
		$amount_2 = isset($jiangpin_detail[2]['now_amount']) ? $jiangpin_detail[2]['now_amount'] : 10;
		$amount_3 = isset($jiangpin_detail[3]['now_amount']) ? $jiangpin_detail[3]['now_amount'] : 30;
		$amount_4 = isset($jiangpin_detail[4]['now_amount']) ? $jiangpin_detail[4]['now_amount'] : 45;
		$amount_5 = isset($jiangpin_detail[5]['now_amount']) ? $jiangpin_detail[5]['now_amount'] : 60;
		...
		$multiple_5 = isset($jiangpin_detail[5]['mutiple']) ? $jiangpin_detail[5]['mutiple'] : 1;		
		$rand = rand(1, $num);
		$begin_1 = 1;
		...
		$begin_5 = $last_4 + 1;
		$last_5 = $begin_5 + intval($amount_5*$multiple_5);	
		if($rand >= $begin_1 && $rand <= $last_1){
			...
			$res = 5;
		}elseif($rand >= $begin_5 && $rand <= $last_5){
			$res = 6;
		}else{
			$res = 1;
		}
