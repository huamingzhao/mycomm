...
<div id="content_text1" class="project_home_content_text project_home_content_text1" style="display:none;">
    ...
    <div id="project_home_content_text_other" style="display:block;">
        ...
        <?php if($article_list) {?>
            ...
            <div class="project_info_other">
                <ul class="news_list" style="border:none;">
                    <?php foreach ( array_slice($article_list,0,6) as $vs_article_list ){
                        ?>
                ...
                <?php if(!empty($article_list) && count($article_list)>6){?>
        ...
        <?php if($industry_article_list) {?>
            <h2><p class="title">行业新闻</p></h2>
            <div class="project_info_other">
                <ul class="news_list" style="border:none;">
                    <?php foreach ( array_slice($industry_article_list,0,6) as $vs_article_list ){
                        ?>
                        <li class="<?php if( $vs_article_list['article_img']!='' ){ echo 'haveimg'; }else{echo 'no_img';}?>">
                            <h3>
                ...
                <?php if(!empty($industry_article_list) && count($industry_article_list)>6){?>
...
<div id="content_text4" class="project_home_content_text" style="display:block">
    <div class="infor_center ad-pt">
        <h1 class="ad_list_title"><?php if( isset( $article ) ){ echo $article['article_name'];  }?></h1>
        <div class="ad_list_time"><?php if( isset( $article ) ){ echo date("Y年m月d日 H:i");$article['article_intime'];  }?>更新    　浏览: <?php echo $pv?> 次</div>
        <div class="ad_list_tags">
            标签：
                <span>
                <?php
                if( isset( $article ) ){
                    if( $article['article_tag']!='' ){
                        $tag_arr= explode(',',$article['article_tag']);
                        foreach ( $tag_arr as $tag_vss ){
                            ?>
                            <a href="<?php echo zxurlbuilder::ptag($tag_vss)?>"><?php echo $tag_vss?></a>；
                        <?php }}}?>
        ...
        <div class="ad_list_text">
            <?php if( isset( $article ) && $article['article_img']!='' ){?>
                <p class="ad_list_text_img"><img width="667" src="<?php echo URL::imgurl($article['article_img'])?>" alt="<?php if( isset( $article ) ){ echo $article['article_name'];  }?>"></p>
            <?php }?>
            <div class="ad_list_text_content">
                <?php
                if( isset( $article ) ){
                    $allow= zixun::setContentApply(true);
                    $show_content= zixun::setContentReplaceByAddKey($article['article_content'],$allow);                                    echo $show_content;
                }?>
            </div>
            <div class="project_home_news_more">
                <font>更多相关新闻</font>
                <a href="<?php echo URL::website('/info/'.$projectinfo->project_id.'.html')?>">查看全部 》</a>
            </div>

            <ul class="project_home_news_more_list">
                <?php foreach( $rsua_list as $vss ) {?>
                    <li><a title="<?php echo $vss['article_name']?>" href="<?php echo zxurlbuilder::porjectzixuninfo($projectinfo->project_id, $vss['article_id'])?>"><?php echo UTF8::substr($vss['article_name'], '0','15')?></a></li>
                <?php }?>
            </ul>
