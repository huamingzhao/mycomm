  ...
  <div class="yjh_item_list" id="yjh_item_list_top">
     ...
     <div class="yjh_item_list_left">
         <ul>            
            ...
            <li><a href="<?php echo urlbuilder:: projectGuide ("people");?>" class="item_list_left_current">按人群找</a></li>
     ...
     <div class="yjh_item_list_right">
         ...
         <div class="yjh_item_list_r_bgcenter">
            ...
            <!--按人群找-->
            <div class="yjh_itemlist_people_title">
            <ul>
            <li><h2><a href="<?php echo urlbuilder:: guideCorwd ("1");?>" <?php if($type == 1 || $type == 0){ echo "class='itemlist_people_currnet'";}?>>女性创业首选项目</a></h2></li>
            <li><h2><a href="<?php echo urlbuilder:: guideCorwd ("2");?>" <?php if($type == 2){ echo "class='itemlist_people_currnet'";}?>>大学生创业首选项目</a></h2></li>
            <li><h2><a href="<?php echo urlbuilder:: guideCorwd ("3");?>" <?php if($type == 3){ echo "class='itemlist_people_currnet'";}?>>农民创业首选项目</a></h2></li>
            <li><h2><a href="<?php echo urlbuilder:: guideCorwd ("4");?>" <?php if($type == 4){ echo "class='itemlist_people_currnet'";}?>>白领创业首选项目</a></h2></li>
            </ul>
            </div>
                                ...
                                <p>
                                  <span>
                                    <a target="_blank"  href="<? echo  $url = urlbuilder::project($val['project_id']);?>">
                                      <img alt="<?= $val['project_brand_name']?>" src="<?if($val['project_source'] != 1) {echo project::conversionProjectImg($val['project_source'], 'logo', $val);} else {echo URL::imgurl($val['project_logo']);}?>" onerror="$(this).attr('src', '<?= URL::webstatic("/images/common/company_default.jpg");?>')">
                                    </a>
                                  </span>
                                </p>
                        ...
                        <?php }?>
                              ...
                              <?if(false){?>
	                              <dd id="searchsendcard_<?php echo $user_id."_".$val['project_id'].'_dd';?>">
	                                <a href="<?= urlbuilder::project($val['project_id'])?>" target="_blank">查看详情</a>
	                                <?if($val['viewProject']){?>
	                                	<span>已收藏</span>
	                                <?}else{?>
	                                <a href="javascript:void(0)" id="watchproject_<?php echo $user_id."_".$val['project_id'];?>" class="button_bb2" com_user_id="<?php echo $val['project_id']; ?>">收藏项目</a>
	                                <?}?>
	                              </dd>
	                           <?php }else{?>
							...
							<h4 class="clearfix">
                             ...
                             <span class="ryl_search_item_intro <?php if(arr::get($val, "projectImg","") == "" || count(arr::get($val, "projectImg")) == 0){echo 'ryl_search_item_no_img';}?>">
                              ...
                              <?php } ?>
                              	 <b>
                              		<?php $sumary_text=$val['product_features'] ? htmlspecialchars_decode($val['product_features']) : htmlspecialchars_decode($val['project_summary']);  
                              	...
                              	<?php }else{?>
                              		<b>
                              		<?php $sumary_text=$val['product_features'] ? htmlspecialchars_decode($val['product_features']) : htmlspecialchars_decode($val['project_summary']);  
                              			if($sumary_text){echo mb_substr(strip_tags(str_replace(' ','',$sumary_text)),0,120,'UTF-8').'...';}
                              		?>
                              	</b> 
                              	<?php }?>
                               </span>
                              ...
                              <b class="ryl_search_item_icon01">
                              	<img src="<?php echo URL::webstatic('images/platform/item_list/icon03.jpg')?>">                          
                                  <?if($val['industryArr']){
                                  		foreach($val['industryArr'] as $keyIarr => $valIarr){?>
                                ...
                                <b class="ryl_search_item_icon02">
                                  <IMG src="<?php echo URL::webstatic('images/platform/item_list/icon04.jpg')?>">
                                  		<?if(arr::get($val, 'project_amount_type', 0) != 0) {?>
                                  			<a target="_blank" href="<?php echo urlbuilder:: fenleiCond(array('pid'=>0,'inid'=>0,'atype'=>0,'pmodel'=>0),array('atype'=>arr::get($val, 'project_amount_type', 0)));?>">
                                  				<i><?php $monarr= common::moneyArr(); echo  arr::get($val, 'project_amount_type') == 0 ? '不限': $monarr[arr::get($val, 'project_amount_type')];?></a>
                                  					</i><?}else{echo "<i>不限</i>";}?> 
                              ...
                              <span class="ryl_search_item_bot">
                               ...
                               <?php }?>
                                <span class="ryl_search_item_bot_right">
                                  ...
                                  <a href="javascript:void(0)" proid="<?=arr::get($val, "project_id")?>" class="zan <?if(!$val['isApproving']){?>searchItemGood<?}?>" title="<?if($val['isApproving']){?>已赞<?}else{?>赞<?php }?>"><?=$val['project_approving_count']?></a>
                                  <?php /*?>
                                  <?if(!$val['isApproving']) {?>
