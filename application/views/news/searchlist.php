<div class="infor_center ad-pt">
            <?php if($total){?>
            <h1 class="ad_tags_tp">包含搜索词“<span><?php echo $keywords;?></span>” 的文章</h1>
            <?php }else{?>
            <h1 class="ad_tags_tp"> 没有找到包含搜索词“<span><?php echo $keywords;?></span>” 的文章，我们为您推荐如下文章：</h1>
            <?php }?>
            ...
            <?php foreach ($list as $k=>$v){
                ...
                ?>
              <li>
                <h3><a target="_blank" href="<?php if($v->parent_id != 29){echo zxurlbuilder::zixuninfo($v->article_id,date("Ym",$v->article_intime));}else{echo zxurlbuilder::porjectzixuninfo(zixun::getZxPorjectId($v->article_id),$v->article_id);}?>" title="">
                <?php
                if (strlen(html_entity_decode(htmlspecialchars_decode($org_art_name)))>80)
                    echo mb_substr(html_entity_decode(htmlspecialchars_decode($v->article_name)),0,20,'UTF-8').'...';
                else
                    echo html_entity_decode(htmlspecialchars_decode($org_art_name));
                ?></a>
                      ...
                      if (strrpos($v->article_tag,',')){
                          ...
                          foreach ($tags as $k=>$tag){
                            if( $k+1==count($tags) ){
                              ...
                              echo '<a href="'.zxurlbuilder::tag(strip_tags($tag)).'" title="'.strip_tags($tag).'">'.$tag.'</a><b>'.$t.'</b>';
                           }
                      }
                      else
                          echo '<a href="'.zxurlbuilder::tag(strip_tags($v->article_tag)).'" title="'.strip_tags($v->article_tag).'">'.$v->article_tag.'</a>';
                  }?>
                  </label>
                ...
                <?php if ($v->article_img!=""){?>
                <?php echo '<div class="infor_newlist_right"><a href="'.zxurlbuilder::zixuninfo($v->article_id,date("Ym",$v->article_intime)).'" title=""><img src="'.URL::imgurl(str_replace( "s_","b_",$v->article_img)).'" alt="'.strip_tags($v->article_name).'" /></a></div>'?>
                <?php }?>
                <div class="clear"></div>
              </li>
              <?php }?>
