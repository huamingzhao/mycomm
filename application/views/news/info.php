...
var tj_pn_id= <?php echo $info['article_id']?>;
         ...
         <div class="infor_center ad-pt">
            <ul class="position">
                ...
                <?php if( $info['parent_id']=='2' ){?>
                    <li><a href="<?php echo zxurlbuilder::xiangmucolumn($parent_col_name)?>"><?php echo $parent_col_name;?></a>></li>
                    <?php if( $col_col_name!='' ){?>
                    <li><a href="<?php echo zxurlbuilder::xiangmucolumn($col_col_name);?>"><?php echo $col_col_name?></a>></li>
                    <?php }?>
                <?php }else{?>
                    <li><a href="<?php echo zxurlbuilder::column($parent_col_name)?>"><?php echo $parent_col_name;?></a>></li>
                    <?php if( $col_col_name!='' ){?>
                    <li><a href="<?php echo zxurlbuilder::column($col_col_name);?>"><?php echo $col_col_name?></a>></li>
                    <?php }?>
                <?php }?>
            ...
            <div class="ad_list_time"><?php echo date("Y年m月d日 H:i",$info['article_addtime'])?>更新    　栏目:<?php echo $col_name?>　<?php if( $info['user_type']=='1' ){?>小编编辑<?php }else{ echo "投稿人:".$author; }?>　围观: <?php echo $pv?> 次</div>
            <div class="ad_list_tags">标签：<?php if( !empty( $tags_arr ) ){foreach ( $tags_arr as $k=>$tgs ){?><span><a href="<?php echo zxurlbuilder::tag($tgs)?>"><?php echo $tgs?></a></span> <?php if( $k+1==count($tags_arr) ){ echo '';}else{ echo ';'; } ?> <?php }}?></div>
            ...
            <div class="ad_list_share">
                <span class="ad_span1 class_ding_detail" id="ding_id_<?php echo $info['article_id']?>" onclick="act_dingcai( <?php echo $info['article_id']?>,'up' )"></span>
                <span class="ad_span2 class_cai_detail" id="cai_id_<?php echo $info['article_id']?>" onclick="act_dingcai( <?php echo $info['article_id']?>,'down' )"></span>

                <span class="ad_span3 person_search_login" onmouseover="getfavoriteeof('<?php echo $info['article_id']?>')" onclick="addfavorite('<?php echo $info['article_id']?>')">收藏<b class="ad_span3_collect_fc" style="display:none" id="favorite_<?php echo $info['article_id']?>"><font id="favorite_eof_<?php echo $info['article_id']?>">收藏成功<!--去收藏--></font></b></span>
