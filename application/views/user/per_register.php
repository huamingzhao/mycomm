...
<?php echo URL::webjs("platform/register/register_per_new.js")?>
...
<!--title标题-->
               <div class="regtitle clearfix">
                 ...
                 <p class="floleft">马上注册并填写基本信息，您会得到更多的关注哟！</p>
               </div>
<!--表单-->
               <div class="ryl_new_regist_cont01">
               <form id="registerForm" method="post" action="<?php echo urlbuilder::register("geren");?>">
                    <!-- <p class="Identityinput"><em>*</em>您的身份：<input type="radio" name="per" value="0" checked="true"><span>个人用户</span><input name="per" value="1" type="radio" style="margin-left: 50px;"><span>企业用户</span></p> -->
                  <p id="input_email_id">
                    <em>*</em>
                    <input type="text" class="login_lk_text01" name="email" id="email" value="<?if(isset($form['email'])){echo $form['email'];}else{?><?}?>" placeholder="邮箱或手机号"  />
                    <span class="ryl_new_regist_tx_text" id="tishi_mail">请输入您常用的邮箱/手机号</span>
                  </p>
                  <p id="input_phoneCode">
                   ...
                   <p><em>*</em><input type="password" placeholder="密码" class="login_lk_text01" name="password" id="passWord" /><span class="ryl_new_regist_tx_text" id="tishi_passswd">6-20位字母、数字或符号</span></p>

                   <p><em>*</em><input type="password" class="login_lk_text01" name="confirm" id="passWordAgain" placeholder="确认密码"/><span class="ryl_new_regist_tx_text" id="tishi_confirm">6-20位字母、数字或符号</span></p>

                   <p id="mail_code"><em>*</em><input type="text" placeholder="验证码" class="login_lk_text02" id="valid_code" name="valid_code" /><img src="<?=common::verification_code();?>" width="95" height="37" class="login_lk_yzm" id="vfCodeImg1"/><a href="javascript:void(0)" id="changeCodeImg" class="login_lk_yzmtext">看不清楚？换一张</a><span id="tishi_code" <?if(isset($error['valid_code'])) {echo ' error';}?>"><?if(isset($error['valid_code'])) {echo $error['valid_code'];}?></span><input type='hidden' name="user_type" value="2"/></p>
                   <!-- 注册错误 -->
                   <?if(isset($error['reg_error'])) {?><p><span class="ryl_new_regist_tx_text error" <?if(isset($error['reg_error'])) {echo 'error';}?>><?if(isset($error['reg_error'])) {echo $error['reg_error'];}?></p><?php }?>

                   <div class="clear"></div>

                   <p id="agree" class="login_lk_wenzi regist_font12"><input type="checkbox" checked/><span>我已经阅读</span><a href="#" style="width:85px;" class="ryl_new_regist_tx_blue messageBox" data-tag="#registerContent" data-title="会员服务条款" data-type="click" data-width="690">注册及服务条款</a><span>，并愿意遵守相关条款规定。</span><span id="tishi_check"></span></p>

                   <div class="clear"></div>

                   <p class="ryl_new_regist_btn"><input  type="image" src="<?php echo URL::webstatic('images/platform/login_new/btn_regist.jpg')?>" class="ryl_new_regist_btn01"/><span>我已经有账号了，我要</span><a href="<?php echo urlbuilder::geren("denglu");?><?php if($to_url){ echo "?to_url=".$to_url; ?><?php }?>">登录</a></p>
                   </form>
                   <div class="clear"></div>
...
<div id="register_box">
    <div class="context">
        ...
        <ul>
            ...
            <li>（2）一句话（<a href="/" target="_blank">http://www.yjh.com</a>），一句话及相关的标识和徵记是通路快建公司在中国和其它国家的商标并受到版权法，商标法和其它知识产权法的保护。未经授权地复制，模仿，使用或发布上述标识，均被禁止。</li>
        </ul>
        <div>补充协议</div>
