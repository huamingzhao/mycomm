...
class Task_DoInsert875ProjectList extends Minion_Task{

	protected function _execute(array $params){
		#php shell php minion --task=DoInsert875ProjectList
		 self::_get875List();
	 ...
	 protected function _get875List(){
	    	...
	    	$arr_data_list = array();
			$arr_data = $Service_Api_Basic->getApiReturn("http://man.875.cn/rest_project/postProjectExactList",array('limit'=>500,"start_date"=>date('Y-m-d',strtotime('-1 day')),"end_date"=>date("Y-m-d")));
			// print_R($arr_data);exit;
			#获取昨天一天发布项目的总数
			$arr_data_list = arr::get($arr_data, "msg");
			if(count($arr_data_list) > 0){
				#循环入库
				if(count($arr_data_list) > 0 ){
					for( $i = 0 ; $i < count($arr_data_list) ; $i++){
						#检查项目是不是存在
						$arr_project = ORM::factory('Project')->where("project_brand_name",'=',trim($arr_data_list[$i]['pro_name']))->where_open()->where("project_source","=",intval(2))->or_where("project_source","=",intval(6))->where_close()->find()->as_array();
						if($arr_project['project_id'] == ""){
							self::Do_project_list($arr_data_list[$i]);
						}else{
							echo "项目名称为:".$arr_data_list[$i]['pro_name']." 已存在"."\n";
							/*$arr_project_url  = $Service_Api_Basic->getApiReturn("http://man.875.cn/rest_project/getMobanInfo",array("pro_id"=>arr::get($arr_data_list[$i],"pro_id")));
							if(isset($arr_project_url['msg']) && $arr_project_url['msg'] && $arr_project_url['msg'][0]['use_moban_url']){
								$bool_project_haihao = self::do_insert_project_haibao(array("project_id"=>$arr_project['project_id'],"url"=>arr::get($arr_project_url['msg'][0], "use_moban_url")));
	...
	protected function  Do_project_list($arr_data){
    	...
    	$arr_project_industry = array();
    	$arr_Projectoutside_data = array();
	    #项目基本信息入库
	    if($arr_data){
	    	//项目表添加开始
	    	#项目来源
	    	...
	    	if(intval(arr::get($arr_data,"invest_money"))){
	    		if(intval(arr::get($arr_data,"invest_money")) == 1){
	    			//5-10万
	    			$arr_project_data ['project_amount_type'] = 2;
	    		}elseif (intval(arr::get($arr_data,"invest_money")) == 2){
	    			//10-20万
	    			$arr_project_data ['project_amount_type'] = 3;
	    		} elseif (arr::get($arr_data,"invest_money") == 5 || arr::get($arr_data,"invest_money") == 3){
	    			//20-30万     30-50万
	    			$arr_project_data ['project_amount_type'] = 4;
	    		}elseif (arr::get($arr_data,"invest_money") == 4){
	    			$arr_project_data ['project_amount_type'] = 5;
	    		}else{
	    			$arr_project_data ['project_amount_type'] = 2;
	    		}
	    	}else{
		...
		unset($arr_project_industry);
		unset($arr_data);
		unset($arr_Projectoutside_data);
	}
	#入库到项目log
